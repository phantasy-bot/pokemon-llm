...
def touch_controls_path_find(mapid, currentPos, screenCoords):
    ...
    print(
        f"POS: {int(currentPos[0])},{int(currentPos[1])}, Translated: {x},{y}, Desination: {int(currentPos[0]) + x},{int(currentPos[1]) + y}"
    )
...
def _astar_find_path(grid, start, end):
    ...
    pq = [(0 + abs(sx - ex) + abs(sy - ey), sx, sy)]
    g_score = {(sx, sy): 0}
    came_from = {}
    ...
    while pq:
        ...
        current_g = g_score[(x, y)]

        for dx, dy, action in dirs:
            nx, ny = x + dx, y + dy

            if not oob(nx, ny) and grid[ny][nx]:
                new_g = current_g + 1
                if (nx, ny) not in g_score or new_g < g_score[(nx, ny)]:
                    g_score[(nx, ny)] = new_g
                    priority = new_g + abs(nx - ex) + abs(ny - ey)
                    heapq.heappush(pq, (priority, nx, ny))
                    came_from[(nx, ny)] = (x, y, action)
    else:
        return None
