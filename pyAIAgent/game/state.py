    ...
    try:
        ...
        for slot in range(count):
            ...
            dex_no, mon_name, type1, type2 = species_map.get(
                internal_id, (None, f"ID 0x{internal_id:02X}", None, None)
            )
...
def get_name_entry_state(sock, menu_state: dict = None) -> dict | None:
    ...
    try:
        ...
        if menu_state:
            ...
            log.info(
                f"name_entry check: menu_item_count={last_item + 1}, cursor=({cursor_x},{cursor_y}), selected={selected}"
            )
        else:
            _flush_socket(sock)
        ...
        # Name entry detection heuristics:
        ...
        if is_preset_menu:
            ...
            # Try to determine which type by checking dialog_text if available
            # For now, use player options as default
            preset_options = preset_options_player
            selected_option = (
                preset_options[selected]
                if selected < len(preset_options)
                else "UNKNOWN"
            )

            log.info(
                f"name_entry: PRESET MENU detected! selected={selected} ({selected_option}), menu_items={last_item + 1}"
            )

            return {
                "is_preset_menu": True,
                "is_keyboard": False,
                "cursor_option": selected_option,
        ...
        if not is_keyboard_likely:
            log.info(
                f"name_entry: NOT detected (last_item={last_item}, cursor=({cursor_x},{cursor_y}))"
            )
            return None
        ...
        return {
            ...
            "tracked_row": tracker_state.get("tracked_row", 0),  # 0-indexed
            "tracked_col": tracker_state.get("tracked_col", 0),  # 0-indexed
            "selected_char": tracker_state.get("selected_char", "A"),
            "row": tracker_state.get("row", 1),  # 1-indexed for human readability
            "col": tracker_state.get("col", 1),  # 1-indexed for human readability
            "is_tracked": tracker_state.get("is_tracked", True),
            "is_name_entry": True,
        }
    except Exception as e:
        ...
        for i in range(16):
            ...
            if picture_id != 0:
                ...
                if 0 <= idx_x < 80 and 0 <= idx_y < 80:
                    ...
                    print(
                        f"DEBUG Sprite {i}: ID={picture_id} Raw=({x_raw},{y_raw}) -> ({idx_x},{idx_y})",
                        file=sys.stderr,
                    )
...
def get_map_state(sock) -> dict:
    ...
    try:
        ...
        return {"last_map_id": last_map, "encounter_rate": encounter_rate}
    except Exception as e:
...
def get_battle_status_flags(sock) -> dict:
    ...
    try:
        ...
        e_status = readrange(
            sock, "0xD067", "3"
        )  # Assuming +5 offset for enemy? Need to verify distance
        ...
        def parse_main_status(byte_val):
            ...
            statuses = []
            if byte_val & 0x07:
                statuses.append(f"Sleep({byte_val & 0x07})")
            if byte_val & 0x08:
...
def get_stat_modifiers(sock) -> dict:
    ...
    try:
        ...
        return {"player_mods": parse_stats(p_stats), "enemy_mods": parse_stats(e_stats)}
    except Exception as e:
    ...
    try:
        ...
        if count > 0:
            ...
            for i in range(count):
                ...
                inventory.append(
                    {"item_id": item_id, "name": item_name, "count": quantity}
                )
    except Exception as e:
...
def get_enemy_pokemon(sock) -> dict | None:
    ...
    try:
        ...
        dex, name, type1, type2 = species_map.get(
            species_id, (0, "Unknown", "Normal", None)
        )
...
def get_event_flags(sock) -> dict:
    ...
    try:
        ...
        flags["have_oaks_parcel"] = (
            oaks_parcel & 2
        ) != 0  # Usually bit 1 for "delivered"? Checking non-zero.
...
def prep_llm(sock) -> dict:
    ...
    try:
        ...
        if loc:
            ...
            log.info(
                f"prep_llm: location = {mapName} ({mid}) at ({x},{y}) facing {facing}"
            )

            rom_path = get_rom_path()
            log.info("prep_llm: generating minimap...")
            t_map = time.time()
            minimap_img = dump_minimal_map(
                rom_path, mid, (x, y), grid_lines=True, crop=MINI_MAP_SIZE
            )
            if minimap_img:
        ...
        if name_entry_state:
            log.info(
                f"prep_llm: NAME ENTRY detected! cursor='{name_entry_state.get('selected_char')}' idx={name_entry_state.get('cursor_index')}"
            )
        else:
            log.debug(
                f"prep_llm: name_entry=None (menu_item_count={menu_state.get('menu_item_count', 0)})"
            )

        total_time = time.time() - t_start
        log.info(
            f"📡 prep_llm DONE: total={total_time:.2f}s location={mapName} movement={movement_state.get('movement_mode')} name_entry={bool(name_entry_state)}"
        )
        ...
        log.error(
            f"📡 prep_llm: Socket ERROR: {e} (errno={e.errno if hasattr(e, 'errno') else 'N/A'})"
        )
        raise
    except Exception as e:
