...
class KeyboardCursorTracker:
    ...
    def apply_action(self, action: str) -> None:
        ...
        for char in action.upper():
            if char == "D":  # Down
                ...
                self.row = (self.row - 1 + self.ROWS) % self.ROWS
            elif char == "R":  # Right
                self.col = (self.col + 1) % self.COLS
            elif char == "L":  # Left
                self.col = (self.col - 1 + self.COLS) % self.COLS
        ...
        if (self.row, self.col) != (old_row, old_col):
            ...
            log.info(
                f"🎹 KeyboardTracker: Moved ({old_row},{old_col}) -> ({self.row},{self.col}) = '{new_char}'"
            )
    ...
    def get_path_to_char(self, target_char: str) -> str:
        ...
        # Vertical movement first (D/U) - optimized for wrapping
        diff_row = (target_row - current_row) % self.ROWS
        if diff_row <= self.ROWS // 2:
            moves.extend(["D"] * diff_row)
        else:
            moves.extend(["U"] * (self.ROWS - diff_row))

        # Horizontal movement (L/R) - optimized for wrapping
        diff_col = (target_col - current_col) % self.COLS
        if diff_col <= self.COLS // 2:
            moves.extend(["R"] * diff_col)
        else:
            moves.extend(["L"] * (self.COLS - diff_col))
    ...
    def get_typing_sequence(self, name: str) -> list[dict]:
        ...
        for char in name.upper():
            ...
            # Calculate moves from simulated position - optimized for wrapping
            diff_row = (target_row - sim_row) % self.ROWS
            if diff_row <= self.ROWS // 2:
                moves.extend(["D"] * diff_row)
            else:
                moves.extend(["U"] * (self.ROWS - diff_row))

            diff_col = (target_col - sim_col) % self.COLS
            if diff_col <= self.COLS // 2:
                moves.extend(["R"] * diff_col)
            else:
                moves.extend(["L"] * (self.COLS - diff_col))
