...
def grid_to_world(
    ...
    Args:
        grid_x: X position in minimap grid
        grid_y: Y position in minimap grid
        player_grid_x: Player's X position in grid (usually center)
        player_grid_y: Player's Y position in grid (usually center)
...
def parse_minimap(minimap_2d: str, world_position: Optional[list] = None) -> dict:
    """
    ...
    for r_idx, row in enumerate(rows):
        for c_idx, char in enumerate(row):
            if char in ("O", "D", "E", ">", "<", "^", "v"):
                ...
                if world_coords:
                    o_tiles.append(
                        f"Grid[{c_idx},{r_idx}] = World[{world_coords[0]},{world_coords[1]}]{dir_hint}"
                    )
                else:
    ...
    for r_idx, row in enumerate(rows):
        for c_idx, char in enumerate(row):
            if char == "N":
                ...
                if world_coords:
                    npc_tiles.append(
                        f"Grid[{c_idx},{r_idx}] = World[{world_coords[0]},{world_coords[1]}]"
                    )
                else:
            ...
            if is_walkable(tile):
                ...
                if above == "B" and below == "B":
                    ...
                    for check_col in range(c_idx + 1, min(c_idx + 10, len(row))):
                        if (
                            is_walkable(get_tile(r_idx, check_col))
                            and get_tile(r_idx - 1, check_col) == "B"
                            and get_tile(r_idx + 1, check_col) == "B"
                        ):
                            passage_width += 1
                        else:
            ...
            if is_walkable(tile):
                ...
                if left == "B" and right == "B":
                    ...
                    for check_row in range(r_idx + 1, min(r_idx + 10, num_rows)):
                        if (
                            check_row < num_rows
                            and c_idx < len(rows[check_row])
                            and is_walkable(get_tile(check_row, c_idx))
                            and get_tile(check_row, c_idx - 1) == "B"
                            and get_tile(check_row, c_idx + 1) == "B"
                        ):
                            passage_height += 1
                        else:
    ...
    for p in passages[:3]:
        if p["type"] == "horizontal_passage":
            passage_strs.append(
                f"🌉 Horizontal path at {p['start']} ({p['width']} tiles wide) - go {p['direction']}"
            )
        else:
            passage_strs.append(
                f"🚶 Vertical path at {p['start']} ({p['height']} tiles tall) - go {p['direction']}"
            )
