...
class NavigationState:
    ...
    suggestion: str
    force_random_action: bool = False
...
class NavigationController:
    ...
    def set_goal(
        ...
        log.info(
            f"🎯 Navigation goal set: {goal_type.value} -> ({target_x}, {target_y}) on {map_name}: {reason}"
        )
    ...
    def _compute_path_for_goal(
        ...
        if goal.map_id != current_map_id:
            log.debug(
                f"Cannot compute path: goal on different map (goal: {goal.map_id}, current: {current_map_id})"
            )
            return None
        ...
        try:
            ...
            cached_path = path_cache.get(
                current_map_id, current_pos, (goal.target_x, goal.target_y)
            )

            if cached_path:
                log.info("Cache HIT")
                goal.computed_path = cached_path
            ...
            if path:
                path_cache.set(
                    current_map_id, current_pos, (goal.target_x, goal.target_y), path
                )
                goal.computed_path = path
                goal.path_computed_at = current_cycle
                log.info(
                    f"🛤️ Path computed: {path[:30]}... ({len(path.split(';')) - 1} moves)"
                )
            else:
    ...
    def update(
        ...
        # Prune stale goals
        self.goal_stack = [
            g for g in self.goal_stack if g.cycles_active < self.MAX_GOAL_CYCLES
        ]
        ...
        if not current_goal:
            ...
            return NavigationState(
                ...
                force_random_action=(stuck_status == "stuck"),
            )

        # Check if goal reached
        distance = abs(current_pos[0] - current_goal.target_x) + abs(
            current_pos[1] - current_goal.target_y
        )
        ...
        should_recompute = current_goal.computed_path is None or (
            current_goal.path_computed_at
            and current_cycle - current_goal.path_computed_at
            >= self.PATH_RECOMPUTE_INTERVAL
        )

        if should_recompute and HAS_PATHFINDING:
            self._compute_path_for_goal(
                current_goal, current_map_id, current_cycle, current_pos
            )
        ...
        suggestion = self._generate_suggestion(
            current_goal, distance, stuck_status, next_moves
        )
    ...
    def _get_stuck_status(
        self, position_history: List[Tuple[str, Tuple[int, int]]]
    ) -> str:
    ...
    def record_goal_failure(self, reason: str) -> None:
        ...
        if self.goal_stack:
            ...
            log.warning(
                f"❌ Goal failure recorded: {reason} (attempts: {self.goal_stack[0].failed_attempts})"
            )
            self._save()
