...
def setup_llm_client(mode: str = None) -> tuple[OpenAI | None, str | None, str | None]:
    ...
    if MODE == "OPENAI":
        ...
        if not api_key:
            log.error(
                "MODE is ZAI_DIRECT but ZAI_API_KEY not found in environment variables."
            )
            return None, None
        try:
        ...
        log.error(
            f"Invalid MODE selected: {MODE}. Set MODE environment variable correctly (e.g., OPENAI, GEMINI, OLLAMA, LMSTUDIO, ZAI, ZAI_DIRECT, OPENCODE)."
        )
        return None, None, None
