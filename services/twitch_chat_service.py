...
TEST_MESSAGES = [
    ...
    "I like shorts!",
    "Where is the S.S. Anne?",
    "Have you beaten Misty yet?",
    "Is that a shiny?",
    "Use a Potion!",
    "Help I'm lost",
    "Can you name the next one Bob?",
    "gg wp",
    "LUL",
    "PogChamp",
    # Sassy/Teasing messages
    "Are you actually playing or just pretending? LUL",
    "Wow you're bad at this game",
    "Why did you pick that move?? Noob AI",
    "Are you an AI? You seem kinda slow lol",
    "Please release your starter it would be funny",
    "You look glitchy today lass",
    "Is this played by a human? You make weird choices",
    "Call your next pokemon Stinky please",
    "Who created you?",
    "Are you real?",
]
...
try:
    ...
    log.warning(
        "twitchio not installed. Twitch chat integration disabled. Install with: pip install twitchio"
    )
...
class TwitchChatService:
    ...
    def __init__(
        ...
        self._is_configured = bool(
            self.bot_username and self.oauth_token and self.channel
        )
        ...
        if self._test_mode:
            log.info(
                f"🧪 Twitch TEST MODE enabled! Will generate {CHAT_TEST_MESSAGES_PER_CYCLE} fake messages per cycle"
            )
        elif not self._is_configured:
            log.warning(
                "Twitch chat not configured. Set TWITCH_BOT_USERNAME, TWITCH_OAUTH_TOKEN, and TWITCH_CHANNEL in .env"
            )
    ...
    def get_pending_mentions(self, since_timestamp: float = None) -> List[ChatMessage]:
        ...
        pending = [
            msg
            for msg in self._pending_mentions
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "pending_mentions": len(
                [m for m in self._pending_mentions if not m.responded]
            ),
    ...
    class TwitchBot(commands.Bot):
        ...
        async def event_message(self, message):
            ...
            if message.author and hasattr(message.author, "badges"):
                ...
                is_sub = (
                    "subscriber" in badges or "founder" in badges or "vip" in badges
                )
