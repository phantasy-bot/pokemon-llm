...
class ExplorationTracker:
    ...
    def __init__(
        self,
        storage_path: str = "data/exploration_data.json",
    ...
    def record_visit(
        self, map_id: int, map_name: str, x: int, y: int, total_walkable: int = 0
    ) -> None:
        """
        ...
        if map_id not in self.maps:
            self.maps[map_id] = MapExploration(
                map_id=map_id, map_name=map_name, total_walkable=total_walkable
            )
    ...
    def get_unexplored_directions(
        self, map_id: int, player_x: int, player_y: int, minimap_2d: str
    ) -> List[str]:
        """
        ...
        Args:
            map_id: Current map
            player_x: Player X position (in minimap center coords)
            player_y: Player Y position (in minimap center coords)
    ...
    def get_context_for_llm(
        self, map_id: int, map_name: str, player_x: int, player_y: int, minimap_2d: str
    ) -> str:
        """
        ...
        unexplored = self.get_unexplored_directions(
            map_id, player_x, player_y, minimap_2d
        )
        if unexplored:
            ...
            if best_dir:
                ...
                parts.append(
                    f"⚡ PRIORITY: Move {best_dir} (chain 5x {button};{button};{button};{button};{button};) to reach unexplored area"
                )
        ...
        if target:
            ...
            parts.append(
                f"SUGGESTED TARGET: Move {dir_str} toward unexplored area ({distance} tiles away, use {steps}+ moves)"
            )
    ...
    def _save(self) -> None:
        ...
        try:
            data = {str(map_id): exp.to_dict() for map_id, exp in self.maps.items()}
